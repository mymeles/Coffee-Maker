...
public class APIOrderTest {
	...
	public void testApiCompleteOrder() throws Exception {
		...
		mvc.perform(put("/api/v1/orders/" + id).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk());
		...
		for (int i = 0; i < inv.getIngredients().size(); i++) {
			assertEquals((int) inv.getIngredients().get(i).getAmount(), (int) (95));
		}
		...
		for (int i = 0; i < 19; i++) {
			inv.useIngredients(ord1.getRecipe());
		}
	...
	public void testApiPickUpOrder() throws Exception {
		...
		mvc.perform(delete("/api/v1/orders/" + ursList.get(0).getId()).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk());

		assertNull(userService.findById(ursList.get(0).getId()).getCustomerOrder());
		
		// check for error with incorrect user id 
		mvc.perform(delete("/api/v1/orders/" + 0).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isNotFound());
		mvc.perform(delete("/api/v1/orders/" + ursList.get(0).getId()).contentType(MediaType.APPLICATION_JSON)).andExpect(status().isConflict());
	...
	public void testApiGetOrder() throws Exception {
		...
		final String black_coffee = mvc.perform(get("/api/v1/orders/" + ordList.get(0).getId())).andDo(print()).andExpect(status().isOk()).andReturn()
				.getResponse().getContentAsString();
		final String mocha = mvc.perform(get("/api/v1/orders/" + ordList.get(1).getId())).andDo(print()).andExpect(status().isOk()).andReturn()
				.getResponse().getContentAsString();
		final String milk_shake = mvc.perform(get("/api/v1/orders/" + ordList.get(2).getId())).andDo(print()).andExpect(status().isOk()).andReturn()
				.getResponse().getContentAsString();
