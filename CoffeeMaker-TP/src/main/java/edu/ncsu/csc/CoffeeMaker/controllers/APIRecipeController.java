...
public class APIRecipeController extends APIController {
    ...
    public ResponseEntity createRecipe ( @RequestBody final Recipe recipe ) {
        ...
        if ( recipe.getIngredients().size() < 1 ) {
            return new ResponseEntity( errorResponse( "Invalid number of ingredients for " + recipe.getName() ),
    ...
    public ResponseEntity editRecipe ( @RequestBody final Recipe recipe ) {
        ...
        if ( null != r ) {
            ...
            service.save( recipe );
            return new ResponseEntity(
                    errorResponse( "Recipe with the name " + recipe.getName() + " successfully updated" ),
                    HttpStatus.OK );
        }
        else {
            return new ResponseEntity( errorResponse( "Recipe can not be updated" ), HttpStatus.NOT_FOUND );
        }
