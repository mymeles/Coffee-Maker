...
public class APIOrderController extends APIController {
	...
	public ResponseEntity placeOrder(@PathVariable final String name, @PathVariable final int amt, @RequestBody final CustomerOrder ord) {
		...
		// Now handle a customer with an order and if there exits then return a message
		...
		if (amt < ord.getRecipe().getPrice()) {
			...
			return new ResponseEntity<String>(successResponse(String.valueOf(amt - (ord.getRecipe().getPrice()))),
					HttpStatus.OK);
		}
	}
...
//	}
	 ...
	 * Inventory
	 */
	public boolean completeOrderHelper(CustomerOrder ord) {
